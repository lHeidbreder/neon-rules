...
\newcommand{\dpeiv}{\mod{Strength to Spare}{The next power the Subject casts within 10 minutes gains a +10.}}
...
\begin{document}
	...
	Please note that a \emph{Psionic} described in this book is very flexible and may outshine even more specialized Protagonists in their own field.
	Also - due to how flexible these rules are - it might demand a lot of imagination from an Adjudicator.
	...
	To even consider casting psionic powers, our Protagonist must first become a \emph{Psionic}.
	Luckily this is accessible to anyone with the necessary connections and funds to buy a \emph{Cassandra} focus implant.\\
	...
	The moment a Subject attains a \hyperref[implant:cassandrapsi-focus]{Cassandra} focus implant,
		he may learn the Psionics skill and with the first level of said skill,
		he gains access to his first psionic discipline.
...
%		\psicomponent{Easy}{+30}{1}{\psitemize{tap someone on the back, send a short psychic message, change hair color, create enough light to read, detect network communication}} %-> this causes formating issues, as the table gets too long for the page
	...
	\begin{tabularx}{\columnwidth}{lrr|X}
		...
		\psicomponent{Average}{+/-0}{+/-0}{2D10}
		\psicomponent{Substantial}{-10}{+1}{10 + 2 per DoS}
		\psicomponent{Powerful}{-20}{+2}{18 + 5 per DoS}
		\psicomponent{Overwhelming}{-30}{+3}{35 + 10 per DoS}
	\end{tabularx}
...
%	Psionics can show advanced restraint, using the bare minimum of power necessary to cast. This reduces the cost by 3 to a minimum of 0 and if it reaches 0, no pside effects can occur either. The drawback is that, since the power is reduced by a lot, no degrees of success can be gained.
	...
	\begin{enumerate}
		\item A fitting characteristic determined by the Adjudicator
			- this may be constitution for most biomancy, and instinct or charisma for telepathy.
			In any case this is exactly what they would use to do the same task without psionic assistance.
	...
	\par
	Whenever a psionic test shows doubles - whether or not the cast was successful - a pside effect is invoked,
		in addition to any critical result.
	The specific effect is taken from a table below, with regards to the discipline cast and the number showing on the dice. \\
	If some effect is unfeasible or does not fit the situation well,
		the next lower number may be applied instead,
		the effect is skipped or something \emph{completely} different may take effect, as per the Adjudicator's discretion.

	\section{Telekinesis}
	\begin{enumerate}
		\setlength\itemsep{-5mm}
		\mod{Another Happy Landing}{After casting the power the Psionic is flung 2D10 meters into the air, crashing down a round later.}
		\dpeii
		\mod{Unlimited Power}{The Psionic starts levitating for a minute, gaining +1 MS for the duration and ignoring difficult terrain.}
		\dpeiv
		...
		\mod{Spacial Distortions}{A sphere of 10m around the Psionic becomes hard to move in for about 15 seconds (5 rounds). All physical actions work as if under water, except for actual swimming.}
		\mod{Power Leak}{The power starts leaking from the Psionic uncontrollably, pushing against her from all sides. The caster becomes unable to move and very hard to be moved for D10 rounds.}
		\dpeix
	...
	\begin{enumerate}
		...
		\mod{Body Swap}{Swap the body with the target for a minute.
			If there were multiple targets, one is chosen at random or all are swapped around, as seen fit by the Adjudicator.
			Cognitive characteristics and all skills are stuck and follow with their mind; all physical characteristics stay with the body.
			\\%
			Should a body die, effects are up to the Adjudicator.
			Common options are the swapped mind getting stuck in the new body,
				or the old mind returning to its body for a split consciousness.
		}
	...
	\begin{enumerate}
		...
		\mod{Turned Off}{One random extremity loses its function as if destroyed for two minutes. If the result would indicate death, the Psionic becomes catatonic instead.}
		\dpeii
		\mod{Really Soft}{The Psionic ignores fall damage and the trauma rule for a minute and gains +20 to all tests concerned with squeezing through small openings.}
		\dpeiv
		...
		\mod{Mixed Senses}{Senses get mixed up for a minute.
			The Psionic loses all actions except her reaction,
				falls prone and cannot get up,
				and all physical actions are at -30.}
		\mod{Augment Rebuff}{RI or MT (whichever is higher) is reduced by D5 for one hour. If it is reduced below 0, the Psionic goes unconscious for the duration and takes D10 damage to every location that has implants requiring RI / MT (whichever was reduced).}
		\dpeix
		\mod{Berserk}{
			All of the Psionic's physical charactistics and Melee Combat Base increase by 20
				and pain-related penalties are ignored for 2D5 rounds.
			She falls into a bloodlusty rage and rushes the Subject in the vicinity that she has the strongest feelings for
				- whatever those feelings may be.
			After the duration the caster drops unconscious for a few hours.}
	\end{enumerate}
	\section{Fluvimancy}
	\begin{enumerate}
		\setlength\itemsep{-8mm}
		...
		\dpeiv
		\mod{Static Field}{An electric field surrounds the Psionic. It is not strong, just enough to be detected and to make hair stand on end.}
		...
		\mod{Man's not hot}{The Psionic is veiled into a mantle of flames for a minute,
				dealing D5 damage with the Spray, Flame and Blast (1) special rules
				centered around her every round.
			The Psionic is immune to all fire-based damage, her equipment however is not.}
		\dpeix
		\mod{Surge}{Overload protection of the focus implant fails at the worst time.
			The Psionic takes a hit with D10+1 damage
				- ignoring cover, armor, shields and injury threshold
				- as well as the \emph{Stun} and \emph{Flame} special rules to the head in a violent display of lightning bolts.
			All Subjects within 3 meters take the same hit but reduced as if it was a regular ranged attack.}
	\end{enumerate}
	\section{Whispers}
	\begin{enumerate}
		\setlength\itemsep{-5mm}
		...
		\mod{Take me with you}{The exchange leaves behind permanent scrap code streams in the psionic's brain which will randomly influence him from time to time - give advice or distract for example.\\
			When the Psionic does something in line with the other consciousness, he gains a +10 bonus.
			If he does something to its dislike or he is simply in its bad books, he gains a -10.
			The "possession" usually last for D10 days, the experience lasts forever.}
	\end{enumerate}
	\section{Blank}
	...
	%activate as necessary, if there are more backgrounds
...
%	\subsection*{In detail}
	\luaimport{components/backgrounds.json}{race.tpl}{bgs}
	
	\section{Boons}
	\vspace{4mm}
	\begin{multicols}{2}
	\boon{Oozing Power}{
		When the Subject gains the ability to use psionics, he figuratively leaks power.
		All psionic powers he casts have their intensity increased by 1 level without additional costs
			but he permanently brings a very obvious effect around him wherever he goes.
		This may be strong winds, a faint whistling or optical illusions;
			whatever it may be, it makes it extremely obvious that he is a psionic to anyone involved.
		}{10}
	\boon{Psionic Aptitude}{
		The Protagonist can acquire an additional psionic discipline.
		He starts the game with the Psionics skill Trained.
		Cannot be taken by Vat-grown or AIs.
	}{12}
	\boon{Psionic Prodigy}{
		The Protagonist can acquire up to two additional psionic disciplines.
		He starts with the Psionics skill at Trained.
		Can only be taken by \emph{Pure} humans.
	}{25}
	\end{multicols}
...
%	\boon{Frail Mind}{become vulnerable to Telepathy (+10 to target)}{-3}
	...
	\section{Skills}
	\edef\lastskill{x}
	\luaimport{components/skills.json}{skill-desc.tpl}{components/skills}

	\section{Abilities}
	\vspace{6mm}
	\begin{multicols}{2}
	\ability{Psionic Discipline}{0}{Psionics Known}{The Subject gains access to her first psionic discipline.}
	\ability{Additional Psionic Discipline I}{600}{Psionic discipline}{
		The Subject gains access to an additional psionic discipline.
		Costs half for those with Psionic Aptitude, and a third for Psionic Prodigies.}
	\ability{Additional Psionic Discipline II}{600}{Psionic Aptitude or Psionic Prodigy}{
		The Subject gains access to a third psionic discipline.
		Costs two thirds for a Psionic Prodigy.}
	\ability{Additional Psionic Discipline III}{600}{Psionics Prodigy}{The Subject gains access to a fourth psionic discipline.}
	\ability{Combat Caster}{500}{Psionics Trained}{Gain +5 to powers with a Cast Time of \emph{Quick} or shorter.}
	\ability{Sixth Sense}{250}{Psionics Trained}{
		When a psi power is cast within 20 meters,
		the Subject may make a Psionics test as a free action.
		On a success he learns the rough direction and distance of the active power.
	}
	\ability{Mage Slayer}{400}{Sixth Sense}{
		The user gains an attack of opportunity
		when a target starts casting a psi power in the user's melee reach.
		\\%
		On a hit the user and the target make an opposed Psionics test.
		If the user wins,
			the target has to end some of his currently active psi powers, one per DoS.
	}
	\end{multicols}
	...
	\supply{Winston's Detector}{
		\emph{Tool}\\
		Allows detection of psionic powers.
		Anyone using this detector can tell when a power is cast within the vicinity (a 20m radius).
        To estimate additional detail requires a test
        	as if the user was a \emph{Blank},
        	using Logic to replace Psionics.}
        {69}
        {0,3}
        {3}
	\supply{Psi Core}
	...
	\def\lastcategory{-}
	\begin{multicols}{2}
		\luaimport{components/implants.json}{implant.tpl}{components/implants}
	\end{multicols}
	
	\section{Chems}
	\vspace{4mm}
	\begin{multicols}{2}
		\luaimport{components/chem.json}{chem.tpl}{components/chem}
	\end{multicols}
