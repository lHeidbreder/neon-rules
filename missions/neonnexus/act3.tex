...
\label{ch:act3}
In the end the Protagonists have the choice:
\begin{sitemize}
...
\label{sec:steal}
The Crew runs a break-and-enter into Hayes' facility.
...
Hyperion employs an amount of security guards
	roughly equal to the amount of Protagonists
	using the \emph{Checkpoint Guard} stat block.
They also employ a \emph{Checkpoint Officer}.
\\%
Each of these is equipped with
gas masks and tear gas grenades
beyond to their normal equipment.
\par%
...
For Hyperion, the chip and Hayes are of similar importance.
If the location is threatened,
	the security guards will attempt to extract
	both Hayes and the chip
	to a safer, more protected location that will be impossible for the Protagonists to penetrate.
The Nexus chip is embedded into room 5's server rack.
Hayes may be moving.

\subsection{Book it}
\label{sec:flee}
In case the Crew decides to take the chip for themselves,
	they need to leave the city immediately.
Once the group is spotted on public surveillance,
	Hyperion's mercs or Dusk's goons
	will engage a chase.
...
\\%
The Crew starts at 30m distance (3 DoS) to their adversaries.
If they reach 120m (12 DoS) to \emph{all} followers,
	they escape.
If the Protagonists stay at or below 0 DoS for two turns in a row,
	they are rammed off the street.

\section{Get revenge}
\label{sec:revenge}
The Crew confronts Dusk
	be it
	in loyalty to the establishment,
	in hopes of being paid by Hyperion,
...
There are also
	\emph{Addicts}
	equal to the amount of Protagonists
...
\begin{exampleblock}
	...
	The crew is expected to prepare
...
The moment she realizes that our Protagonists did not come to bring the chip to her,
