...
\par \vspace{-5mm}
While possessed, the Subject can still use an action to reduce the Demon's DoS and the Demon can still use its to increase the connection.
...
\begin{itemize}
	...
	\item To act at all, the Subject must take an Opposed Courage test against the Demon during Declaration.
...
\begin{itemize}
	...
	\item Since the binding is so strong, the demon cannot take over control.
		...
		\\%
		Once each session one successful test will fail instead, as chosen by the Adjudicator,
