...
%\usepackage[noxcolor]{svg}
\newsavebox\ltmcbox

\usepackage{../core-rulebook/config/packages}
%FIXME: only place that still requires the old importer
\directlua{require("../scripts/lua/csv-importer.lua")}
...
\newcommand{\mutbodylocation}[1]{%
	\textbf{\large#1 Mutations}\\
	\vspace{2mm}
	\TextField[width=0.3\textwidth, name=mut#1, multiline]{}
	\par
}
...
\def\charfield#1{ \TextField[width=15mm, name=#1]{} }
...
\pagebreak
%addons
%do shells need a page? AI does not need the normal HP
\input{components/pageaddon_psionics}
\newpage
\input{components/pageaddon_cosmic-horrors}
