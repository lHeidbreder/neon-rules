\pagebreak %sections ignore needspace if there is a subsection directly below
...
When an Adversary passes through or leaves the Subject’s control area,
	the Subject may make a single attack as a free action,
...
One attack of opportunity may be made against any one Adversary once per combat round.
\subsection*{Called Shot (-size modifier)}
The Subject targets a specific location (see p.\pageref{sizestable}).
This does not require an ability and may modify another melee maneuver.
\subsection*{Disengage (free action)}
\label{action:disengage}
When disengaging, no attacks are allowed to be made that round but no attacks of opportunity are invoked when passing through other Subjects' area of control.
...
\subsection*{Cleave (-10 per target)}
A half circle of steel, making room, pushing back multiple Adversaries at once.
This maneuver can target up to three combatants,
	so long as they are all within the Subject’s control area.
...
\\%
By the Adjudicator's discretion
...
The next combat action the Subject takes against this opponent gains a bonus equal to the voluntary penalty.
...
Intercepting a \emph{Charge} is at an additional penalty equal to half the Subject’s courage below 80 but deals additional damage equal to the attacker’s movement speed.
\\%
Cannot be performed without the corresponding ability,
...
To initiate a hold, the Subject makes a grappling attack,
	that may be dodged or parried like normal attacks.
If the Subject wins, a hold is engaged.
\\%
The attacker gains bonuses equal to X to every action against the target, for as long as the hold remains.\\
To break free the target makes a grappling check which may be opposed by the Subject as a reaction. If the target wins, it breaks free.
\subsection*{Throw}
The Subject attempts to lift the target off the ground and forcefully put it back.\\
The Subject makes a grappling attack from a \emph{Hold}.
If he wins,
...
\subsection*{Lock}
The Subject attempts to prevent his opponent from moving freely.
Arm locks, leg locks and head locks are obvious and common,
	but bear hugs also fall into this category.
\\%
To initiate a lock, the Subject makes a grappling attack from a \emph{Hold}.
\\%
While the lock is active,
	the Subject may use a reaction to interfere with any action taken by the target:
	the Subject takes a grappling check as an action
...
While holding an opponent in a Lock of some kind, the Subject may choose to initiate a lever hold as an action.
...
To initiate a lever hold,
	the Subject has to have his target in a \emph{Lock} already
	and makes a grappling attack.
If the Subject is successful,
	a location is determined (not necessarily randomly)
	and a lever hold is initiated.
\\%
While the lever hold is active,
	the Subject deals 1D5+\emph{Melee Damage Bonus} damage to the location,
	ignoring armor.
...
To initiate a choke hold the Subject makes a grappling attack,
	either from a \emph{Hold},
	or onto an unsuspecting target.
If the Subject wins, the choke hold is initiated.\\
...
If he wins by at least 3 DoS, he may break free completely.
\subsection*{Move}
Moving while grappling is hard, even more so if only one combatant intends to move.\\
Whenever a Subject in the grappling wants to move
	(and his opponents don't...),
	he makes a grappling attack as a free action,
	which may be \emph{Opposed} as a free action.
If the Subject wins, the target is moved up to the difference of DoS in meters
...
After an attack that does not include intentional movement the Subject
...
\begin{exampleblock}
	\itshape
	Adjudicator's Note:\\
	These rules only really make sense using battle maps,
	and even then they are potentially easily overlooked.
	Note that these rules specifically are especially optional.
\end{exampleblock}
