...
\subsection*{Aim}
The Subject designates a target and takes aim.
For every action aiming the Subject
...
\end{exampleblock}
Additional Aim bonuses from equipment (e.g. mods or implants) are applied
once during the first action spent Aiming.
\par
This maneuver does not require an ability.
\subsection*{Brace}
A Subject using a heavy weapon or launcher (that is not mounted)
...
\subsection*{Reload}
The Subject reloads his firearm.
Depending on the weapon used, this can take varying amounts of time
	which usually may be interrupted if the Subject needs to dodge.
\\%
He makes a Reload check.
If the Subject is reloading two weapons simultaneously
	- such as when dual wielding -
	the test takes a -40 penalty.
...
A heavy or launcher weapon can be reloaded with the help of another Subject instead.
In such a case both Subjects will be occupied for the duration
	but both Subjects may reload as a combined action, adding their DoS.
\\%
Failed tests and critical results once again do nothing.
\begin{exampleblock}
	Two Subjects are reloading an M2 Brass Storm (18 action reload).
	\\%
...
The Subject takes a single shot against a target.
Aim bonuses are not lost from this action.
This does not require an ability.
\subsection*{Burst (-10)}
The Subject takes a relatively controlled burst of fire against a target.
...
The Subject shoots in the general direction of his targets,
	expecting them to keep their heads down so as to not lose them.
Whether a hit is scored or not, all Subjects (hostile or otherwise) within an area of
...
\begin{exampleblock}
	...
	\\%
	She rolls an attack to suppress her targets, scoring 1 DoS.
	She now suppresses an area that is roughly
		3 meters in radius (or 6 meters diameter)
		and one Adversary in the area takes one hit (as she scored a DoS).
	\\%
	All Adversaries in this area need to make a
		-12 Courage test
...
This does not require an ability and may modify another ranged maneuver.
\subsection*{Reaction: Dodge (-10)}
A Subject may attempt to dodge primitive ranged attacks like bows and thrown weapons, as well as move out of blast ranges.\\
