...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{known:} The character has a grasp of this field’s basics.
			This grants a +10 bonus to associated skill checks.
		\item \textbf{trained:} The character had either some formal training or a natural knack for this field.
			This grants a +20 bonus to associated skill checks.
		\item \textbf{experienced:} The character had extensive training in this field.
	...
	Depending on the Anthrotypes and Backgrounds taken, the character will already have some increases.
	...
	\end{sitemize}
	Any Education can at most double the character's skill bonus.
	\begin{exampleblock}
		\itshape
		Jaina has 50 Intelligence, the characteristic the GM deemed most fitting.
	...
	\vspace{-8mm}
	\section{Degradation}
	For every full 20\% of location HP lost, the character takes a -5 penalty to all tests when using this body part.
	...
	\paragraph{Blood loss}is another way a character may die.
	When characters start \emph{bleeding lightly} as determined by degradation or GM discretion,
    ...
    When \emph{bleeding severely} the character only has their Injury Threshold in combat rounds (or three times that in seconds) before unconsciousness sets in.
	...
	\end{exampleblock}
	With basic medical knowledge (a Bio-Maintenance check +40, if in doubt) a character can stop another from bleeding out. That character needs to be right next to the patient and cannot perform any other actions other than holding the wound shut.
	...
	After \emph{implant surgery} a character will have to recover for a week.
	If he is to act during this time he suffers penalties at the GM’s discretion, at least
	\begin{sitemize}
	...
	\begin{exampleblock}
		...
		If deemed unnecessary by the GM, recovery from surgery can safely be ignored.
	\end{exampleblock}
	...
	When faced with any type of overstimulation
		(e.g. blinding lights, loud noises) the character makes a test at +20.
	The exact test depends on the type of stimulus
		...
		representing the character's resilience.
	\\%
	...
	When the time reaches 25 combat rounds,
		the character is very likely to lose the given sense permanently.
	\\%
	...
	\\%
	As a rule of thumb a character is deafened for one combat round for every shot fired within 2 meters or 4 meters indoors.
	...
	\end{exampleblock}
	\paragraph{Suffocation}- commonly through drowning or smoke inhalation - is an often underestimated threat to most characters.
	\label{hazards:suffocation}
	If a character consciously holds her breath, she can hold it for three times her Constitution in seconds (or her Constitution in combat rounds).
	...
	\paragraph{Fire} and the burns associated with it are a minor threat to characters.
...
%
	...
	\section{Prefabs}
	If you want to skip character creation and dive right into play, choose one of the prefab characters.
	...
	Player characters are not the average Joe; they are heroes, villains, the protagonists of their story.
	...
	These are the first steps of character creation, which every character needs to have.
	\par
	...
	If those have prerequisites that the character does not fulfill,
		they remain dormant until the character fulfills the prerequisites.
	...
	\section{Abilities}
	Some abilities might be interesting for the character as well.
	...
	Lastly, a character needs some equipment.
	The amount of money a character starts with is equal to 350 plus their charisma. \\
