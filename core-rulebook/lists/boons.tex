%TODO: \luaimport{lists/boons.csv}{}{lists/boons}
\section{Boons}
\label{sec:boonlist}
\vspace{10mm}
\begin{multicols}{2}
...
\boon[each]{Big Spender}{The character gains additional equipment worth up to \(2 * Ch * level\) of this boon. The money can only be spent at character creation and unused credits are wasted. Equipment may as always be restricted by the GM. Cannot be combined with Destitute.}{3}
...
\boon{Heroic Characteristic}{
	Whenever a character succeeds at a test against his Heroic Characteristic,
	he scores an additional 2 DoS.
	This requires an Aptitude in the same characteristic already.
}{6}
\boon{Iron Resilience}{Degradation from wounds is reduced by one step.
	Bleeding starts as normal.}{10}
...
\boon{Outstanding Sense}{Grants +20 to tests based on the chosen sense.
	Counts as a Trained Sense for the purposes of prerequisites, the bonuses will however still add up.
	Not compatible with Impaired Sense for the same sense.}{7}
...
\boon{Poison Resistance}{When the character starts being effected by a chem, he may make a Con test. On a success he ignores the effects.}{4}
\boon{Privileged Access}{
	The character gains a 25\% discount for license-free equipment made by a certain company.
	During character creation this bonus is 50\% instead to account for sales in the past.
	\\%
	He also has access to limited, rare equipment made and/or sold by the company.
	Any roll for Availability (see p. \pageref{sec:availability}) gains a +1 (again specific to the trade partner).
	\\%
	Requires Good Reputation with that company / salesman.}{7}
...
\boon{Social Chameleon}{The character has extreme ease fitting into any social group. Trying to pass as someone of a different class, sex, origin, and so on gain +40.}{8}
\boon{Stoic}{Halves net penalties or doubles net bonuses on fear tests.}{18}
\boon{Superior Balance}{Tests to prevent falling over are at a +30. Improves on Balance and doesn't stack.}{16}
\boon{Tough As Nails}
	{The character can continue acting while bleeding until completely bled out.
	He never starts \emph{severe bleeding}.}
	{12}
\boon{Unbridled Rage}{Any check to enter a berserk or frenzy state is at a +20 and any check to resist or return from such a state is at a -20.}{3}
\boon{Unrivaled Flexibility}{Agile without peer, it’s easy for the character to squeeze through tight spaces.
	He gains a +30 when attempting to release himself from holds or to escape from being tied up.}{15}
